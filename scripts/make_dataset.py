...
class DatasetMaker:
    def __init__(self, data_dir, train_dir, test_dir, training_sample=400, n_cut=5, crop_size=256, modulo_bits=8,
                 ref_bits=12, multi_fold_for_training=False, **random_exposer_args):
        self.data_dir = data_dir  # contains the original 512*512 hdr images
    ...
    def _cut(self, img, name):
        imgs = dict()
        for i in range(self.n_cut):
            h_offset = random.randint(0, 512 - self.crop_size)
            w_offset = random.randint(0, 512 - self.crop_size)
            imgs[name + "_" + str(i)] = img[h_offset:h_offset + self.crop_size, w_offset:w_offset + self.crop_size, :]
        return imgs
    ...
    def make(self, mode):
        ...
        for hdr_file in tqdm(mode_data_list, ascii=True):
            ...
            for name, img in self._cut(hdr_img, hdr_name).items():
                log += "--------------------\nname: {}\n".format(name)
                random_exposer = RandomExposer(img, **self.random_exposer_args)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Make dataset: from RGB(H, W, C) or grayscale(H, W, 1)")
    ...
    parser.add_argument("--training_sample", default=400, type=int,
                        help="number of training sample")  # the rest will be used as test sample
    parser.add_argument("--n_cut", default=5, type=int, help="number of cut per image")
    ...
    # for test, we don't need to crop, just keep original 512*512 resolution
    dataset_maker.crop_size = 512
    dataset_maker.n_cut = 1
